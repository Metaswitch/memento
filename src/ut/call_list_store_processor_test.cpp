...
public:
  CallListStoreProcessorTest()
  {
    ...
    _clsp = new CallListStoreProcessor(&_load_monitor, _cls, 0, 2, 604800);
  }
...
public:
  ...
  {
    _cls = new MockCallListStore();
    _clsp = new CallListStoreProcessor(&_load_monitor, _cls, 4, 1, 604800);
  }
...
// get_call_fragments_sync can return
...
TEST_F(CallListStoreProcessorTest, CallListIsCountNeededNoLimit)
{
  ...
  bool rc =_clsp->_thread_pool->is_call_trim_needed("impu", timestamp, 0);
  ASSERT_FALSE(rc);
}
...
TEST_F(CallListStoreProcessorTest, CallListWrite)
{
  ...
  _clsp->write_call_list_entry("impu", "timestamp", "id", CallListStore::CallFragment::Type::BEGIN, "xml", 0);
  sleep(1);
}
...
{
  EXPECT_CALL(_load_monitor, request_complete(_)).Times(1);
  EXPECT_CALL(*_cls, write_call_fragment_sync(_, _, _, _, _)).WillOnce(Return(CassandraStore::ResultCode::CONNECTION_ERROR));
  _clsp->write_call_list_entry("impu", "timestamp", "id", CallListStore::CallFragment::Type::BEGIN, "xml", 0);
  sleep(1);
}
...
TEST_F(CallListStoreProcessorWithLimitTest, CallListIsCallTrimNeeded)
{
  ...
  ASSERT_TRUE(rc);
  ASSERT_EQ(timestamp, "20020530093011");
}
...
{
  EXPECT_CALL(*_cls, delete_old_call_fragments_sync(_,_,_,_)).WillOnce(Return(CassandraStore::ResultCode::OK));
  _clsp->_thread_pool->perform_call_trim("impu", "timestamp", 123, 0);
}
...
TEST_F(CallListStoreProcessorWithLimitTest, CallListPerformCallTrimCassError)
{
  ...
  _clsp->_thread_pool->perform_call_trim("impu", "timestamp", 0, 0);
}
